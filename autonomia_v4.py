#!/usr/bin/env python3
# =====================================================================================
# autonomia_v4.py  -- CORRE EN LA RASPBERRY DEL ROBOT (nodo ROS2). Autonomia A BORDO.
...
# Correr:  export ROS_DOMAIN_ID=67 && python3 ~/autonomia_v4.py
...
W_ROT  = 1.0            # giro EN EL SITIO en ROTATE (decidido).
LEFT_BIAS = 0.0

# --- Umbrales de modo (histeresis DRIVE <-> ROTATE) ---
D_BLOCK = 0.55          # frente < esto -> ROTATE (se detiene y gira). Antes de llegar a la pared.
D_FREE  = 0.80          # frente > esto -> vuelve a DRIVE.
D_SLOW  = 0.90          # arriba de esto, velocidad plena.
D_EMERG = 0.20          # frente < esto (o ciego) = peligro -> ROTATE inmediato.

# --- Hueco transitable ---
D_CLEAR    = 0.70       # un rayo es "libre" si supera esto.
ROBOT_PASS = 0.42       # ancho minimo que debe caber el robot.
FRONT_DEG       = -90.0 # CALIBRADO: caja al frente leyo ~-90 deg (verificado izq/der/frente OK).
FRONT_HALF_DEG  = 28.0  # medio cono frontal para decidir DRIVE/ROTATE.
...
class AutonomiaV4(Node):
    def __init__(self):
        super().__init__("autonomia_v4")
        ...
        self.get_logger().info("=== autonomia_v4 (DRIVE/ROTATE decididos + giro/odom) ===")
    ...
    def mejor_hueco(self, scan, use_stick=True):
        ...
        while i < n:
            if pts[i][1] > D_CLEAR:
                ...
                dw = pts[j][0] - pts[i][0]
                if 2.0 * dmin * math.sin(max(dw, 0.0) / 2.0) >= ROBOT_PASS:
    ...
    def decide(self, scan, dt):
        ...
        w = clamp(KP_HEADING * theta_f + KD_HEADING * thetadot_f + LEFT_BIAS, -W_MAX, W_MAX)
        # velocidad: plena si frente despejado; se apaga al acercarse a D_BLOCK (no repta).
        v = LIN * clamp((front - D_BLOCK) / (D_SLOW - D_BLOCK), 0.0, 1.0)
        self._log(f"[DRIVE] front={front:.2f} rumbo={math.degrees(theta_f):.0f} v={v:.2f} w={w:.2f}")
        return v, w

    # ------------------------------- Bucle de control -------------------------------
    def control_step(self):
        if not rclpy.ok():           # contexto ya cerrado (shutdown) -> no publiques
            return
        scan = self.scan
        ...
        if USE_STAMPED:
            ...
            m.header.frame_id = "base_link"     # algunos controladores exigen frame_id no vacio
        ...
        try:
            ...
            pass                     # contexto invalido durante shutdown: ignorar

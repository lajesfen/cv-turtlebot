#!/usr/bin/env python3
# =====================================================================================
# autonomia_v18.py -- CORRE EN LA RASPBERRY DEL ROBOT (nodo ROS2). Autonomia A BORDO.
...
class AutonomiaV18(Node):
    def __init__(self):
        super().__init__("autonomia_v18")
        ...
        self.get_logger().info("=== autonomia_v18 (v17 + puerta-real en mejor_hueco, arcos angostos; YOLO/gaps intactos) ===")
    ...
    def decide(self, scan, dt):
        ...
        if self._sign_fresh():                            # obedece la senal
            ...
            if SIGN_INVERT: lado = -lado                  # v18: corrige ESPEJO (si aplica)
            # v18.3: SOLO huecos hacia ADELANTE (|centro| <= FWD_CONE) -> nunca gira ~180 hacia atras (el bug del pasillo de regreso).
            fwd = [g for g in gaps if abs(g[0]) <= math.radians(FWD_CONE_DEG)]
            if fwd:
                # entre los de ADELANTE, el MAS del lado de la senal:
                #   derecha (lado<0) -> MENOR centro (mas a la derecha);  izquierda (lado>0) -> MAYOR centro.
                #   Ej: en "frente(0) o izquierda(+70)", derecha elige 0 = SEGUIR DE FRENTE (evita la izquierda).
                h = min(fwd, key=lambda g: g[0]) if lado < 0 else max(fwd, key=lambda g: g[0])
                self.get_logger().info(f">>> SENAL {'IZQ' if lado>0 else 'DER'} -> hueco mas a ese lado (adelante), centro={math.degrees(h[0]):.0f} deg")
            # si no hay hueco hacia adelante: no fuerza nada -> geometria
        if h is None:

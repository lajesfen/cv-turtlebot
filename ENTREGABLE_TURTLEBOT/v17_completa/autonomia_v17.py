#!/usr/bin/env python3
# =====================================================================================
# autonomia_v17.py -- CORRE EN LA RASPBERRY DEL ROBOT (nodo ROS2). Autonomia A BORDO.
...
SIGN_TTL          = 10.0   # una senal de giro en el buffer expira por TIEMPO (s)...
...
MEM_CELL      = 0.25    # tamano de celda (m). Mas chico = memoria mas fina pero mas sensible a la
...
W_VISIT       = 0.60    # peso del castigo por rumbo que apunta a celdas YA visitadas (en el costo del hueco).
...
class AutonomiaV17(Node):
    def __init__(self):
        super().__init__("autonomia_v17")
        ...
        self.get_logger().info("=== autonomia_v17 (memoria anti-loop + bumper lado real + senal por distancia + rescate rapido) ===")
    ...
    def btn_cb(self, msg):
        ...
        if pressed and not self._btn_prev:
            ...
            if self.armed: self.visit.clear()      # v17: memoria fresca por intento
    ...
    def _consume_sign(self):
        # devuelve la senal de giro del BUFFER si es FRESCA (tiempo+distancia) y la consume; si no, None.
        if self._sign_fresh():
            d = self.sign_dir
            self.sign_dir = None
            return d
    ...
    def decide(self, scan, dt):
        ...
        if len(gaps) >= 2 and self._sign_fresh():
            ...
            cand = [g for g in gaps if (g[0] >= 0) == (lado > 0)]
            if cand:
                h = min(cand, key=lambda g: abs(g[0]))     # el mas alineado de ese lado
                self._consume_sign()
                self.get_logger().info(f">>> BIFURCACION: obedezco SENAL -> {'IZQ' if lado>0 else 'DER'}")
        if h is None:
